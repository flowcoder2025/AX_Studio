...
export default function BlockSortable({ blocks, onReorder, onToggle, onEdit }: BlockSortableProps) {
...
function SortableBlockItem({
  ...
  return (
    ...
    >
      ...
      {/* Order number */}
      <span className="text-[10px] text-gray-400 w-4 text-center flex-shrink-0">{index + 1}</span>
      ...
      <button className="flex-1 min-w-0 text-left" onClick={onEdit}>
        ...
        {!hasData && (
          <span className="text-[10px] text-orange-400">데이터 없음</span>
        )}
      </button>

      {/* Source badge */}
      <span className={`badge ${badge.className} text-[9px] flex-shrink-0`}>{badge.label}</span>
